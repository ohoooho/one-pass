...
type memoryDB struct {
	mu   sync.Mutex
	data map[string]onepass.Secret
}

func newMemoryDB() *memoryDB {
	return &memoryDB{data: map[string]onepass.Secret{}}
}

func (db *memoryDB) Get(key string) (onepass.Secret, error) {
	db.mu.Lock()
	defer db.mu.Unlock()
	s, ok := db.data[key]
	if !ok {
		return onepass.Secret{}, fmt.Errorf("not found")
	}
	return s, nil
}

func (db *memoryDB) Status(key string) (onepass.Secret, error) {
	return db.Get(key)
}

func (db *memoryDB) Put(key string, secret onepass.Secret) error {
	db.mu.Lock()
	defer db.mu.Unlock()
	db.data[key] = secret
	return nil
...
func (db *memoryDB) Update(key string, fn func(onepass.Secret) (onepass.Secret, error)) error {
	db.mu.Lock()
	defer db.mu.Unlock()
	s, ok := db.data[key]
	if !ok {
...
func testPublicKey(t *testing.T) string {
	...
	entity, err := openpgp.NewEntity("onepass-test", "", "test@example.com", cfg)
	if err != nil {
...
func TestSecretRequestLifecycle(t *testing.T) {
	...
	encrypted, err := onepass.Encrypt(strings.NewReader("hunter2"), "decryptionkey")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSecretRequestRotateKey(t *testing.T) {
	...
	encrypted, err := onepass.Encrypt(strings.NewReader("hunter2"), "key")
	if err != nil {
		t.Fatal(err)
	}
	fulfillBody, _ := json.Marshal(map[string]string{"message": encrypted})
...
func mustEncrypt(plaintext string) string {
	encrypted, err := onepass.Encrypt(strings.NewReader(plaintext), "key")
	if err != nil {
		panic(err)
	}
	return encrypted
...
func TestSecretRequestFulfillWithFile(t *testing.T) {
	...
	encrypted, err := onepass.Encrypt(strings.NewReader(strings.Repeat("f", 5000)), "key")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSecretRequestFulfillKindPolicy(t *testing.T) {
	shortMessage, err := onepass.Encrypt(strings.NewReader("hunter2"), "key")
	if err != nil {
		t.Fatal(err)
	}
	longMessage, err := onepass.Encrypt(strings.NewReader(strings.Repeat("x", 5000)), "key")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSecretRequestExpired(t *testing.T) {
	...
	data, _ := json.Marshal(r)
	db.data[requestKeyPrefix+id] = onepass.Secret{Message: string(data)}
...
func TestSecretRequestConcurrentFulfill(t *testing.T) {
	...
	for i := range messages {
		encrypted, err := onepass.Encrypt(strings.NewReader(fmt.Sprintf("secret-%d", i)), "key")
		if err != nil {
			t.Fatal(err)
		}
