...
func TestMemcachedUpdate(t *testing.T) {
	...
	if err := m.Update(key, func(s onepass.Secret) (onepass.Secret, error) {
		return s, nil
	}); err != ErrKeyNotFound {
		t.Fatalf("expected ErrKeyNotFound, got %v", err)
	}

	if err := m.Put(key, onepass.Secret{Message: "0", Expiration: 3600}); err != nil {
		t.Fatalf("error in Put(): %v", err)
	}
	...
	for i := 0; i < writers; i++ {
		...
		go func() {
			defer wg.Done()
			for {
				err := m.Update(key, func(s onepass.Secret) (onepass.Secret, error) {
