 ...
 * - Done:    green check, gray connector
 * - Future:  gray ring, faded title
 *
 * ≤768px: stacks vertically (one bullet per row).
 */
  ...
  return (
    ...
    >
      {steps.map((step, idx) => {
        ...
        const circleSize = 'w-9 h-9 md:w-10 md:h-10 text-sm md:text-base';
        ...
        return (
          <li
            key={step.id}
            className="flex md:flex-col md:flex-1 md:items-center md:text-center gap-3 md:gap-2"
            data-testid={`step-bar-item-${step.id}`}
            aria-current={isCurrent ? 'step' : undefined}
          >
            <div className="flex items-center md:flex-col md:items-center md:w-full">
              <div
              ...
              {!isLast && (
                <div
                  className={`hidden md:block h-0.5 flex-1 mx-1 transition-colors duration-200 ${
                    isDone ? 'bg-[#7DD3C0]' : 'bg-[#E0E6F0]'
                  }`}
                  aria-hidden="true"
                />
              )}
              {/* Mobile vertical connector (not last step) */}
              {!isLast && (
                <div
                  className={`md:hidden w-0.5 h-3 ml-4 ${
                    isDone ? 'bg-[#7DD3C0]' : 'bg-[#E0E6F0]'
                  }`}
                  aria-hidden="true"
                />
            ...
            <div className="md:max-w-[10rem] md:mx-auto">
              <div
