...
func TestCLI(t *testing.T) {
	...
	msg := "onepass CLI integration test message"
...
func TestCLIFileUpload(t *testing.T) {
	...
	msg := "onepass CLI integration test file upload"
	...
	// Note onepass decrypt currently always prints the content to stdout. This
...
func TestDecryptWithoutCustomKey(t *testing.T) {
	viper.Set("decrypt", "https://one-pass.ohoooho.com/#/c/21701b28-fb3f-451d-8a52-3e6c9094e7ea")
	err := decrypt(nil)
	if err == nil {
...
func TestDecryptWithInvalidUrl(t *testing.T) {
	viper.Set("decrypt", "https://one-pass.ohoooho.com")
	err := decrypt(nil)
	if err == nil {
		t.Fatal("expected invalid url error, got none")
	}
	want := `Invalid onepass decrypt URL: unexpected URL: "https://one-pass.ohoooho.com"`
	if err.Error() != want {
		t.Fatalf("expected %s, got %s", want, err.Error())
	}
}
...
func TestDecryptWithUnconfiguredUrl(t *testing.T) {
	...
	want := `Unconfigured onepass decrypt URL, set --api and --url`
	if err.Error() != want {
		t.Fatalf("expected %s, got %s", want, err.Error())
	}
}
...
func TestSecretNotFoundError(t *testing.T) {
	...
	want := `Failed to fetch secret: onepass server error: unexpected response 404 Not Found: Secret not found`
...
func newTestServer(t *testing.T) (*httptest.Server, func()) {
	db := &testDB{data: make(map[string]onepass.Secret)}
	y := server.Server{
		DB:                  db,
